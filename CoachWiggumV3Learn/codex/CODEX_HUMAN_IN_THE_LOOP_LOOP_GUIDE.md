...
The loop now includes view subcommands for monitoring and reviewing runs:

### `view past` — list completed runs

Lists all completed JSONL files with timestamps and sizes. Pick one to view its final output.

```bash
./loop.sh view past
./loop.sh view .codex-run/build-20260320-143512.jsonl
```

### `view future` — follow the current loop

Tails the latest JSONL file through the renderer so you can watch a running loop from another terminal:
...
This is the same two-terminal pattern as the Claude Code version. The Codex version uses the existing `render_codex_exec_stream.py` renderer rather than an interactive dashboard, but the workflow is identical.

### Extending with a richer dashboard

The Claude Code version of this loop includes an interactive dashboard with:

- multi-run comparison grids with cost tracking
- tool breakdown charts and phase timelines
- follow mode with automatic idle detection
- teaching content (phase explanations, tool tips, learning summaries)

If you want the same richness for Codex, you can adapt `render_coach_dashboard.py` from the ClaudeRalph bundle to parse Codex JSONL events instead of Claude events. The core pattern (phase detection, tool counting, summary box) is the same.
