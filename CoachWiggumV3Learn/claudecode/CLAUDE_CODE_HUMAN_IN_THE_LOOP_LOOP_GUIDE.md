...
## View modes: monitoring and reviewing runs

The loop now includes a view system for monitoring live sessions and reviewing past runs.

### `view past` — browse completed runs

Shows an interactive grid of all completed JSONL files with:

- timestamp, mode (plan/build/coach), tool count, files changed, duration
- real API cost extracted from JSONL result events
- file change heatmap (most-changed files across runs)
- grand totals row (total cost, tools, duration, success rate)
- drill-down into any run with full teaching content

### `view future` — follow a live session

Watches `.claude-run/` for new JSONL files and tails them in real-time:

- shows a live dashboard with spinners, phase banners, and tool events
- when an iteration completes, shows summary and waits for the next
- accumulates a running stats grid across all observed iterations
- stops when the session goes idle (30s with no new file)
- Ctrl+C shows partial summary at any time

This is designed for a two-terminal workflow:
...
- **Tool breakdown chart** — horizontal bar graph of tool usage (Read, Edit, Bash, etc.)
- **Phase timeline** — proportional bar showing time spent investigating vs coding vs validating
- **Cost & tokens** — real `total_cost_usd`, token counts, per-model breakdown (Opus/Sonnet/Haiku)
- **Teaching blocks** — "What Just Happened" and "What to Review Next" for quick scanning
