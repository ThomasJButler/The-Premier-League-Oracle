...
#   ./loop.sh coach                      # Dashboard viewer for latest run
#   ./loop.sh coach path/to/file.jsonl   # Replay a specific run
#
# Common runs:
#   ./loop.sh plan 1
...
#   ./loop.sh coach
#     Interactive dashboard — tails the latest .claude-run/*.jsonl
#   ./loop.sh coach .claude-run/build-20260319-143512.jsonl
#     Interactive dashboard — replays a specific JSONL file
...
case "$MODE" in
  ...
  build)
    ...
    ;;
  coach)
    # Standalone dashboard viewer — tails or replays a JSONL file
    DASHBOARD_RENDERER="scripts/render_coach_dashboard.py"
    if [[ ! -f "$DASHBOARD_RENDERER" ]]; then
      echo "Missing dashboard renderer: $DASHBOARD_RENDERER"
      exit 1
    fi
    RUN_DIR=".claude-run"
    if [[ ! -d "$RUN_DIR" ]]; then
      echo "No .claude-run/ directory found. Run a plan or build first."
      exit 1
    fi
    # Allow specifying a JSONL file directly, otherwise find the latest
    if [[ -n "${2:-}" && -f "${2:-}" ]]; then
      LATEST_JSONL="$2"
    else
      LATEST_JSONL="$(ls -t "$RUN_DIR"/*.jsonl 2>/dev/null | head -1)"
    fi
    if [[ -z "$LATEST_JSONL" ]]; then
      echo "No JSONL files found in $RUN_DIR/"
      echo "Run a build or plan first, then come back."
      exit 1
    fi
    exec python3 "$DASHBOARD_RENDERER" "$LATEST_JSONL"
    ;;
  *)
    if [[ "$MODE" =~ ^[0-9]+$ ]]; then
      PROMPT_FILE="PROMPT_build.md"
      ...
      echo "Usage: ./loop.sh [plan|build|coach] [max_iterations] [coach|homer]"
      exit 1
    fi
    ;;
esac
