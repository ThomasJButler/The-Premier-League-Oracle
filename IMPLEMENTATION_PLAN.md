...
- **Test count baseline:** 851 tests across 103 files + 10 Playwright cases (svelte-check 0/0).
...
- **2026-05-25 — K1j shipped (Predict GW button + bulk-persist helper).** `/today` KPI strip gains a `[Predict GW NN]` chip wired to a new `lib/bulkPersistGameweekPredictions.ts` (idempotent, deps-injectable so tests don't load OptimizedPredictor). 851/851 tests (+5), svelte-check 0/0. Next auto-gated leaf in queue order: **K1d-β.1** (derive full top-6 standings on demand). Remaining K1-polish after that: K1b-γ.1 (live persona-voiced match analysis).
