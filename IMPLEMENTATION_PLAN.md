...
- [ ] **K0l-β — Settings persistence** *(Manual-gated, MVP gate — commit landed, sweep pending)* → `k0.12` — Shipped: real `<input type="password">` for both Football-Data + Anthropic keys with `bind:value` + `oninput` localStorage persistence (`football_data_api_key`, `ANTHROPIC_API_KEY_STORAGE_KEY`), SSR-guarded hydration in `onMount`. `PrefToggle` primitive at `lib/components/settings/PrefToggle.svelte` (role="switch", `data-pref-toggle` / `data-pref-checked` / `data-pref-switch` markers, two-tone slider via `var(--ink)` / `var(--paper-inset)`). Notifications panel renders 3 toggles (match-start / model-edge / broadsheet-ready) persisted as JSON blob at `kicker:notifications`. **Discovery:** `/api/chat` does NOT yet read the BYOK Anthropic key — `getAnthropic()` is env-only. Wiring this is a follow-up (logged below). The key is persisted client-side now; forwarding is K1d/K2c territory. **Sweep checklist still applies:** `docs/the-kicker-spec/sweep-k0l.md` covers both α + β.
...
- **2026-05-12 — K0l-β terminator condition for the next loop:**
  - **The bug to fix:** user-reported on the Settings page — both API key fields render as static placeholders (`paste key — saved to this device only` / `bring-your-own-key (Touchline tier)`) with no functional `<input>`. K0l-α deliberately shipped them as disabled stubs with the microcopy "Persistence wires up in K0l-β. Configure via VITE_FOOTBALL_DATA_API_KEY for now." K0l-β makes both inputs functional.
  - **Required deliverables:**
    1. Convert `data-input-football-data` and `data-input-anthropic` stubs into real `<input type="text">` (or `<input type="password">` with a show/hide eye for paranoia) — proper Svelte `bind:value` + `oninput` that writes to `localStorage` on each keystroke (or `onblur` if you want to debounce; either is fine).
    2. Football-Data key persists at localStorage key `football_data_api_key` (already the contract in `services/api/footballData.ts:120`). On load, the input hydrates with the current value (SSR-guarded).
    3. Anthropic key persists at `ANTHROPIC_API_KEY_STORAGE_KEY` from `$lib/constants` (verify the exact string). `/api/chat` already reads this for the bring-your-own-key path.
    4. Ship `PrefToggle` primitive (two-tone slider switch — proper Svelte component, NOT the inline-`useState`-in-`.map()` hook-rule violation from the JSX prototype) + 3-toggle list on the Notifications panel (match-start / model-edge / broadsheet-ready), persisted at localStorage key `kicker:notifications` as a `{matchStart, modelEdge, broadsheetReady}` JSON shape.
    5. The "LOCAL ONLY" pill in the Connections panel header is correct and should stay — accurate UX (keys never leave the browser except to their target API).
  - **Auto-validate:** `npm run check --prefix frontend` 0/0; `npm run test --prefix frontend -- --run` all-green. ~6 new tests expected (API key round-trip via `data-input-*` markers, `PrefToggle` click toggles persisted JSON, notifications panel renders 3 toggles, no betting copy).
  - **Commit:** `feat(K0l-β): Settings API-key persistence + PrefToggle notifications`. Manual-gated; do NOT auto-flip `[x]`, do NOT auto-apply `k0.12`. After β commits, the human pastes their Football-Data key into the live `/settings` input, hard-reloads, confirms it survives, then I apply `k0.12` and flip both K0l-α + K0l-β to `[x]` together.
  - **Reuse contracts:** `PunditPickerCard` (K0h) untouched. `Rule` (K0c) untouched. New: `PrefToggle.svelte` in `frontend/src/lib/components/settings/` (~30 LoC, `bind:checked` + label slot, ink-when-on / paper-inset-when-off colour rule per the JSX prototype).
  - **SSR safety:** input default values must be read SSR-guarded — `typeof localStorage !== 'undefined' ? localStorage.getItem(KEY) : ''` directly in the route's `<script>` (or via `onMount` if simpler). Don't crash SSR.
