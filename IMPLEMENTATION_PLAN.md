...
- **Test count baseline:** 844 tests across 102 files + 10 Playwright cases (svelte-check 0/0).
...
- [ ] **K1a-β.3.1 — Teach personas to emit FIXTURE/CHEERS tokens** *(Auto-gated, server prompt edit)* → no tag — Append a short "When you cite a stat that's gloriously useless, wrap as `[[CHEERS:stat|label|text]]`; when you reference a specific upcoming fixture, wrap as `[[FIXTURE:HOMETLA-AWAYTLA]]`" block to `lib/server/buildSystemPrompt.ts` so Claude actually produces these tokens organically — K1a-β.3's parser already handles them but no persona is currently prompted to emit them. ~10 LoC + 2 tests asserting the instruction block lands in every persona's composed prompt.
