...
<div class="value-bets-container">
  ...
  {:else if matchEntries.length === 0 && !error}
    <div class="bg-slate-50 dark:bg-slate-800 rounded-xl p-12 text-center">
      ...
      <p class="text-slate-600 dark:text-slate-400">No upcoming matches found</p>
      <p class="text-sm text-slate-500 dark:text-slate-500 mt-2">Check back when matches are scheduled</p>
    </div>
  {:else}
    <!-- Odds Entry Section -->
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-lg p-6 mb-6">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-2">
          ...
          <h2 class="text-lg font-bold text-slate-900 dark:text-white">Enter Bookmaker Odds</h2>
        </div>
        <div class="flex items-center space-x-4">
          <div class="flex items-center space-x-2">
            <span class="text-sm text-slate-500 dark:text-slate-400">Bankroll:</span>
            <div class="relative">
              ...
              <input
                ...
                bind:value={bankroll}
                min="1"
                class="w-24 pl-6 pr-2 py-1.5 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
              />
            </div>
          </div>
        ...
        {#each matchEntries as entry, idx}
          <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
            <div class="flex flex-col lg:flex-row lg:items-center gap-4">
              ...
              <!-- Date -->
              <div class="text-xs text-slate-500 dark:text-slate-400 lg:w-24 flex-shrink-0">
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">Home</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
                    ...
                    class="w-16 px-2 py-1.5 text-sm text-center bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                  />
                </div>
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">Draw</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
                ...
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">Away</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
            ...
            {#if entry.showAdvanced}
              <div class="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 flex items-center gap-3 flex-wrap" transition:slide>
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">Over 2.5</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
                ...
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">Under 2.5</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
                ...
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">BTTS Yes</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
                ...
                <div class="flex flex-col items-center">
                  <label class="text-xs text-slate-400 mb-1">BTTS No</label>
                  <input
                    type="number"
                    step="0.01"
                    min="1.01"
      ...
      <div class="mt-6 flex items-center justify-between">
        ...
        <button
          ...
          class="btn btn-primary flex items-center space-x-2 px-6"
        >
          {#if analysing}
      ...
      <div class="bg-white dark:bg-slate-900 rounded-xl shadow-lg p-4 mb-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
          <div class="flex items-center space-x-6 text-sm">
            {#if lastAnalysis}
              <div>
                <span class="text-slate-500 dark:text-slate-400">Last analysis:</span>
                <span class="font-medium ml-1">{formatDate(lastAnalysis)}</span>
              </div>
            {/if}
            <div>
              <span class="text-slate-500 dark:text-slate-400">Value bets found:</span>
          ...
          <!-- Market Filters -->
          <div class="flex items-center space-x-2">
            <span class="text-sm text-slate-500 dark:text-slate-400">Market:</span>
          ...
          <div class="bg-white dark:bg-slate-900 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
            ...
            <div class="p-6">
              ...
              <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 mb-4">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">Kelly Recommended Stake</p>
