...
<script lang="ts">
  ...
  import { TrendingUp, Zap, RefreshCw, BookmarkPlus, Check } from 'lucide-svelte';
    ...
    try {
      ...
      for (const match of upcomingMatches) {
        ...
        results.push({
          ...
          insights: prediction.insights.slice(0, 2)
        });
      }
  ...
  // Recalculate stakes when bankroll changes
...
<div class="max-w-lg mx-auto space-y-6" data-testid="suggested-bets">
  <!-- Bankroll input -->
  <div class="card-glass p-4">
    <label for="sb-bankroll" class="block text-sm font-medium text-foreground mb-1.5">
      Your Bankroll
    </label>
    <div class="relative">
      ...
      <input
        ...
        type="number"
        bind:value={bankroll}
        on:change={saveBankroll}
        min="1"
        step="10"
        class="w-full pl-7 pr-3 py-2.5 rounded-lg border border-border bg-muted text-foreground text-sm"
      />
    </div>
    <p class="text-xs text-muted-foreground mt-1">Shared with Kelly Calculator</p>
  </div>
