...
**Frontend (current, on `butler-model`)**
...
- **Tag track:** `k0.1`–`k1.0` applied (MVP complete + tagged 2026-05-12). K1a–K1i (Oracle, Match-detail, Live, Insights/Archive, Column/Broadsheet, Notifications/Search, Roster, Landing, Rumours) are all **feature-complete on `butler-model`**; the `k1.1`–`k1.9` tags are applied for the sign-off-clean slices, with K1b/K1d/K1e/K1f still awaiting their human sweep before tagging.
- **Engine:** the Butler model (time-decayed Dixon-Coles) was swapped in July 2026, replacing the retired five-model ensemble — the current live tip of `butler-model`. See `docs/prediction-engine.md` + `docs/butler-python-roadmap.md`.
- **Remaining:** production `main` cutover (deferred until post-K2) + Phase 2 — K2a mobile responsive pass, K2b ElevenLabs audio (key-blocked), K2c Stripe paywall (legal-blocked), K2d Print Run (deferred). Planned repo split to `thekicker`: `docs/thekicker-migration-plan.md`.
