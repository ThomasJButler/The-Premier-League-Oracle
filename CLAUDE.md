...
- `train_free_tier.py` - Free-tier training script (XGBoost + stacked OvR ensemble + LR baseline, chronological split, rolling CV via `--cv`)
...
- Frontend has **507 Vitest tests** across 32 test files, all passing (was 404)
- 43 Playwright E2E tests across 6 spec files (0 skipped), run in 3 viewports = 123 total executions
- 17 components have unit tests (Dashboard, BettingHistory, ChatBot, LiveMatches, Predictions, Settings, KellyCalculator, ValueBets, AccumulatorBuilder, StandingsTable, TopScorers, SeasonStats, Help, MatchList, LiveTicker, ApiSetupWizard, MatchEventToast) — 3 components untested (Header, SidebarNav, Sidebar — layout/navigation only)
...
- ~~`free_tier_features.py`: `_latest_elo()` fallback path walked backwards from END of ALL data, leaking future ELO ratings into features~~ **FIXED:** added `before_date` parameter that skips matches at or after the cutoff; `_elo_features()` passes `match_date` as `before_date`. All 45 feature tests pass including `test_no_future_data_used`
...
- `backtest.test.ts:156-174`: expected value `0.525` is mathematically correct — `VALUE_ODDS_MARGIN` (1.05) cancels during normalisation, so the value is the same with or without the margin
...
- Test quality: `backtest.test.ts:154-178` investigated — `0.525` is correct (VALUE_ODDS_MARGIN cancels in normalisation, not a bug); `value.test.ts` 3 weak assertions fixed; backend happy-path test for `/predict/free` added (P5ae)
- ~~CI gaps: No coverage enforcement, no linting step, no E2E tests in pipeline~~ **FIXED:** CI runs `test:coverage` with enforced thresholds (60/65/65/60), ESLint for frontend (`npm run lint`), ruff for backend (`ruff check`). E2E tests still not in CI pipeline (needs `npx playwright install`). ESLint flat config at `frontend/eslint.config.js`, ruff config at `backend/pyproject.toml`
