...
- ~~No CI/CD~~ **FIXED:** `.github/workflows/ci.yml` runs type check, unit tests with coverage enforcement (60/65/65/60 thresholds), and production build on push/PR to `main` and `v3.0-*` branches. Backend Python tests (62 tests via pytest) now also run in CI (P5c). Node version reads from `.nvmrc` instead of hardcoded `20` (P5g)
