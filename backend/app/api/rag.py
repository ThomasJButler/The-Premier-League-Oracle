...
historical match DataFrame (2,191+ matches), and builds an augmented
system prompt so the LLM can give data-grounded responses instead
of hallucinating statistics.
...
class QueryIntent:
    ...
    __slots__ = ('teams', 'query_type', 'season', 'last_n', 'stat_type')
...
def parse_intent(text: str) -> QueryIntent:
    ...
    if any(kw in lower for kw in ['vs', 'versus', 'against', 'head to head', 'h2h', 'record against']):
...
def _query_season_table(df: pd.DataFrame, season: Optional[str], teams: list[str]) -> str:
...
def build_rag_prompt(
    ...
    # Query DataFrame for relevant data
    data_context = ""
    total_matches = 0
    seasons: list[str] = []
